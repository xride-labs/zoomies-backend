 ...
 */
const RIDE_RETENTION_DAYS = parseInt(process.env.RIDE_RETENTION_DAYS || "30", 10);
 ...
 */
export async function cleanupOldRides(): Promise<{ deleted: number; errors: string[] }> {
  const errors: string[] = [];
  let deleted = 0;
  ...
  try {
    ...
    console.log(`[Ride Cleanup] Found ${ridesToDelete.length} rides eligible for deletion`);
    ...
    console.log(`[Ride Cleanup] Completed. Deleted: ${deleted}, Errors: ${errors.length}`);
 ...
 */
export async function cleanupOrphanedMedia(): Promise<{ deleted: number; errors: string[] }> {
  const errors: string[] = [];
  let deleted = 0;
 ...
 */
export async function updateRideStatuses(): Promise<{ updated: number; errors: string[] }> {
  const errors: string[] = [];
  let updated = 0;
